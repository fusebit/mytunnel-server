...
const argv = optimist
  ...
  .options("tunnel-ssl-ca", {
    describe:
      "the certificate authority chain for the ssl cert that is used to encrypt the tunnel",
  })
  ...
  .options("auto-generate-cert", {
    describe:
      "enabling this flag will automatically generate the server certs for you",
  })
  .options("override-tunnel-ip", {
    describe:
      "override the ip to connect to when establishing sockets, usually meant to be used if you are using a external load balancer",
  })
  .options("auto-discover-tunnel-ip", {
    describe:
      "automatically descover the tunnel public ip via the AWS checkip endpoint",
  })
  .options("secret", {
    describe:
...
if (argv["auto-discover-tunnel-ip"]) {
  await axios.get("http://checkip.amazonaws.com").then((response) => {
